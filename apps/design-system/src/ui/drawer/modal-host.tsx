...
import type { ModalEntry, FeedbackModalEntry, ConfirmationModalEntry, InputModalEntry, CustomModalEntry, ModalPosition, ModalKind } from './drawer-store';

/* ============================================================
   ModalHost — renders imperative modals queued via DrawerService.
   Mount once at app root.
   ============================================================ */

const KIND_ACCENT: Record<ModalKind, { rule: string; confirm: string; confirmText: string }> = {
  info: {
    rule: 'bg-[var(--text-primary)]',
    confirm: 'bg-[var(--text-primary)] text-[var(--neutral-0)] hover:opacity-80',
    confirmText: 'text-[var(--neutral-0)]',
  },
  success: {
    ...
    <div className="relative bg-[var(--surface-raised)] border border-[var(--text-primary)] shadow-[0_24px_48px_rgba(24,22,19,0.24)] overflow-hidden">
      ...
      <div className="flex items-center justify-end gap-2 px-[22px] py-3.5 border-t border-[var(--border-default)] bg-[var(--surface-sunken)]">
        <button
          type="button"
          onClick={confirm}
          className={`h-8 px-4 font-ui text-[13px] font-medium border-0 cursor-pointer transition-colors rounded-[4px] ${acct.confirm}`}
        >
...
function ConfirmationModal({ entry }: { readonly entry: ConfirmationModalEntry }) {
  ...
  return (
    <div className="relative bg-[var(--surface-raised)] border border-[var(--text-primary)] shadow-[0_24px_48px_rgba(24,22,19,0.24)] overflow-hidden">
      {isDestructive && <span className="absolute left-0 top-0 bottom-0 w-[3px] bg-[var(--danger-icon)]" />}
      {!isDestructive && <span className={`absolute left-0 top-0 bottom-0 w-[3px] ${acct.rule}`} />}
      <div className="flex items-baseline gap-3 px-[22px] py-4 border-b border-[var(--text-primary)] bg-[var(--surface-sunken)]">
        <h2 className="m-0 font-serif text-[19px] font-medium tracking-[-0.012em] text-[var(--text-primary)]">{entry.title}</h2>
        <button type="button" onClick={dismiss} className="ml-auto bg-transparent border-0 cursor-pointer text-[var(--text-tertiary)] hover:text-[var(--text-primary)] p-0">
          <X size={16} />
      ...
      <div className="flex items-center gap-2 px-[22px] py-3.5 border-t border-[var(--border-default)] bg-[var(--surface-sunken)]">
        <button
          type="button"
          onClick={dismiss}
          className="h-8 px-3.5 font-ui text-[13px] font-medium border border-[var(--border-default)] text-[var(--text-tertiary)] cursor-pointer bg-transparent hover:text-[var(--text-primary)] hover:border-[var(--text-primary)] transition-colors rounded-[4px]"
        >
        ...
        <button
          type="button"
          onClick={confirm}
          className={`h-8 px-4 font-ui text-[13px] font-medium border-0 cursor-pointer transition-colors rounded-[4px] ${
            isDestructive
              ? 'bg-[var(--danger-icon)] text-[var(--neutral-0)] hover:opacity-80'
              : acct.confirm
          }`}
        >
          {entry.confirmButtonText}
...
function ModalShell({ entry }: { readonly entry: ModalEntry }) {
  ...
  const flex = positionClasses(position);
  const dlg = dialogClasses(position);
  ...
  return (
    <div
      className={`fixed inset-0 z-[9000] flex justify-center bg-black/20 ${flex}`}
